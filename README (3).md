...
| 7.5% stock-out rate on high-margin A-class items | £110,000 in lost annual sales |
...
- **Scope 3 Carbon Calculator** — DEFRA 2025-compliant emission calculations per shipment
...
│   └── pages/
│       ├── home.py                  # Project overview + live KPIs
│       ├── inventory.py             # Inventory dashboard
│       ├── sustainability.py        # Carbon reporting
│       ├── scenario.py              # What-If planning
│       └── cfo.py                   # Executive financial summary
│
├── dashboard/
│   ├── pbi_*.csv                    # 8 pre-aggregated Power BI tables
...
│
└── docs/
    ├── executive_summary.md         # 1-page ROI summary
    ├── case_study.md                # Full consulting case study
    ├── technical_spec.md            # Architecture decisions explained
    └── README_final.md              # Extended portfolio documentation
```
...
| 2 | SQL Star Schema & ETL pipeline | ✅ Complete |
| 3 | Python analytics engine | ✅ Complete |
| 4 | Streamlit web application | ✅ Complete |
| 5 | Power BI dashboard layer | ✅ Complete |
| 6 | Testing suite | ✅ Complete |
| 7 | Documentation & portfolio assets | ✅ Complete |
...
| `fact_purchase_orders.csv` | 2 years of purchase orders | 795 |
| `fact_inventory_snapshots.csv` | Monthly inventory snapshots | 1,200 |
| `ALL_SUPPLIERS_combined_raw.csv` | Dirty supplier ingest feed (~12% errors) | 795+ |
| `_error_manifest.csv` | Error log across 7 error types | 91 |
...
| Metric | Baseline | After System |
|---|---|---|
| Total Stock Value | — | £788,000 |
| Excess stock value | £280,000 | £238,000 target (-15%) |
| A-class stock-out rate | 7.5% | Target: <3% |
| Carbon reporting | 0% automated | 100% automated |
| Weekly reporting time | 12 hours manual | Under 10 minutes |
| Scope 3 emissions tracked | — | 282 tonnes CO2e (2yr) |
| Urgent orders | 17 | Reduced by earlier ROP trigger |
...
python python/green_rop.py
python python/carbon_calculator.py
...
| A-class stock-out rate | 7.5% | Under 3% |
| Carbon reporting | 0% automated | 100% automated |
| Weekly reporting time | 12 hours manual | Under 10 minutes |
